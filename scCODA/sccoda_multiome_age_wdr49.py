...
# ==============================================================================
# WHAT THIS IS
#   The authoritative contrast 7, age at death, fitted on our dataset alone with
#   WDR49+ astrocytes kept as their own population.
#
# WHY IT EXISTS
#   An earlier version of contrast 7 pooled both cohorts and reported WDR49+
#   astrocytes as credibly depleted in C9orf72: beta -0.663, inclusion probability
#   0.9974, the strongest effect anywhere in the table. It is an artefact, and this
#   script is what established that.
#
#   All ten external donors who have age at death recorded are C9orf72, and every one
#   of them carries zero WDR49+ nuclei, because the label does not transfer to their
#   data. Separately, restricting to donors with an age recorded had already dropped
#   17 of 42 controls, and the dropped ones were disproportionately the WDR49-poor.
#   So the comparison put a control arm stripped of its zeros against a C9orf72 arm
#   one third composed of them.
#
#   Refitting on our cohort alone drops that inclusion probability from 0.9974 to
#   0.4927 and the contrast returns nothing credible. Do not quote the old number.
#
# WHAT IT PRODUCES
#   .../sccoda/reviewer_multiome_age_wdr49/OurCohort_multiome/All_AgeAtDeath/
#
# WHERE IT LANDS IN THE SUPPLEMENTARY TABLE
#   Contrast (vii). SLURM job 52251115, 19 minutes 24 seconds.
#
# HOW IT DIFFERS FROM ITS SIBLING SCRIPT
#   sccoda_multiome_continuous.py runs the same three contrasts on the same cohort
#   but sets HARMONIZE_WDR49 to true, which merges WDR49+ into astrocytes and leaves
#   21 cell types. That version cannot answer the WDR49 question at all, which is why
#   this one exists. Note the setting below is False here.
#
#   Unlike the refit script, this one filters on study explicitly. That is
#   deliberate: contrast 7 genuinely could have included external donors, since ten
#   of them do have an age recorded, so the restriction is a decision and is written
#   down as one.
#
# THE SAMPLER WORKAROUND BELOW
#   Three earlier attempts died mid-sampling with a CPython fatal error and no
#   traceback, at unpredictable points between 11 and 72 percent. The cause is
#   tqdm's progress-bar callback racing with JAX's multi-threaded CPU backend. The
#   monkeypatch and the single-thread environment variables are what make this run
#   reproducible. Leave them in place.
# ==============================================================================
...
# Kills tqdm's host callback. Without this the run dies mid-sampling with a
# CPython fatal error and no traceback, at an unpredictable point.
...
OUTDIR  = "/oak/stanford/scg/lab_mpsnyder/johnck/Projects/RK/ALS_multiome/sccoda/reviewer_multiome_age_wdr49"
DS_LABEL = "OurCohort_multiome"
# Explicit cohort restriction, unlike the refit script's incidental one. Ten
# external donors do have an age recorded, so including them was a real
# option; excluding them is a decision, and it is written down as one.
STUDY_VALUE = "ALS"
...
NUM_SAMPLES   = 10000
NUM_WARMUP    = 1000
# False here, and that is why this script exists at all. Keeping WDR49+
# astrocytes separate gives 22 cell types and lets the contrast speak to the
# population the paper is built on. Its sibling sets this true and cannot.
HARMONIZE_WDR49 = False
...
CONTRASTS = [c for c in CONTRASTS if c.number == 7]
